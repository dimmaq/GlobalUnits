...
uses
  Windows, Classes, SysUtils, RTLConsts, uAnsiStrings, AcedCommon,
  AcedStrings, uGlobalFunctions, uGlobalTypes, uGlobalConstants
  {$IFDEF UNICODE}
    , AnsiStrings
  {$else}
  {$ENDIF}
...
type
  ...
  private
    ...
    //---
    procedure _CreateBuffer;                    {$IFNDEF DEBUG} inline; {$ENDIF}
    procedure _Write(P: Pointer; L: Integer);   {$IFNDEF DEBUG} inline; {$ENDIF}
    procedure _WriteLine;                       {$IFNDEF DEBUG} inline; {$ENDIF}
    procedure _FlushBuffer;                     {$IFNDEF DEBUG} inline; {$ENDIF}
    procedure _FlushStream;                     {$IFNDEF DEBUG} inline; {$ENDIF}
  public
    ...
    {$ENDIF}
    procedure Write(Value: TCharArray; Index, Count: Integer);       {$IFDEF UNICODE} override;{$ELSE}overload;{$ENDIF}
    ...
    {$ENDIF}
    procedure WriteLine(Value: TCharArray; Index, Count: Integer);   {$IFDEF UNICODE} override;{$ELSE}overload;{$ENDIF}
    //---
    procedure WriteStrings(AStrings: TStrings); {$IFDEF UNICODE}overload;{$ENDIF}
    {$IFDEF UNICODE}
      procedure WriteStrings(AStrings: TAnsiStrings); overload;
    {$ENDIF}
    //---
...
uses uGlobalVars, uGlobalFileIOFunc, uAnsiStringUtils;
...
constructor TAnsiStreamWriter.Create(const AFileName: string; ACreateNew: Boolean);
var
  h: THandle;
  f: TFileStream;
begin
  SafeForceDirectories(ExtractFileDir(AFileName));
  h := OpenFileWrite(AFileName, not ACreateNew, True);
...
procedure TAnsiStreamWriter.Write(Value: TCharArray; Index, Count: Integer);
var
  b1,b2: Boolean;
  I,L,C: Integer;
  {$IFDEF UNICODE}
...
procedure TAnsiStreamWriter.WriteLine(Value: TCharArray; Index, Count: Integer);
begin
...
{$IFDEF UNICODE}
procedure TAnsiStreamWriter.WriteStrings(AStrings: TAnsiStrings);
var j: Integer;
begin
